...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'offers' AND column_name = 'offer_name') THEN
    ALTER TABLE offers ADD COLUMN offer_name TEXT;
    RAISE NOTICE '✅ 添加 offer_name 字段到 offers';
  ELSE
    RAISE NOTICE '⏭️  offer_name 字段已存在于 offers';
  END IF;
END $$;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'offers' AND column_name = 'target_language') THEN
    ALTER TABLE offers ADD COLUMN target_language TEXT;
    RAISE NOTICE '✅ 添加 target_language 字段到 offers';
  ELSE
    RAISE NOTICE '⏭️  target_language 字段已存在于 offers';
  END IF;
END $$;
...
VALUES ('009_add_offer_name_and_language.pg')
