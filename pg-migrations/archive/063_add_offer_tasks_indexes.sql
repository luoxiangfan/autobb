/**
 * Migration 063: Add indexes to offer_tasks table for performance optimization (PostgreSQL)
 *
 * 优化目标:
...
-- PostgreSQL特定: 添加BRIN索引用于时间序列数据
-- BRIN索引非常适合按时间排序的大表,占用空间小
CREATE INDEX IF NOT EXISTS idx_offer_tasks_created_at_brin
ON offer_tasks USING BRIN (created_at);
