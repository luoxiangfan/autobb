...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'offers' AND column_name = 'review_analysis') THEN
    ALTER TABLE offers ADD COLUMN review_analysis TEXT;
    RAISE NOTICE '✅ 添加 review_analysis 字段到 offers';
  ELSE
    RAISE NOTICE '⏭️  review_analysis 字段已存在于 offers';
  END IF;
END $$;
...
VALUES ('013_add_review_analysis_field.pg')
