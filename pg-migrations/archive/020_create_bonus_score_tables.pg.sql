...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'offers' AND column_name = 'industry_code') THEN
    ALTER TABLE offers ADD COLUMN industry_code TEXT;
    RAISE NOTICE '✅ 添加 industry_code 字段到 offers';
  ELSE
    RAISE NOTICE '⏭️  industry_code 字段已存在于 offers';
  END IF;
END $$;
...
VALUES ('020_create_bonus_score_tables.pg')
