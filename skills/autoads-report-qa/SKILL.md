...
description: 基于 AutoAds 日报和策略建议进行问答、归因与行动建议。适用于“解释今天ROAS变化”“为什么发布失败”“该怎么调预算/出价/素材”这类报表解读场景。优先读取 `/api/openclaw/reports/daily`、`/api/openclaw/strategy/recommendations`、`/api/openclaw/commands/runs`，输出结论+证据+下一步。
---
...
2. `/api/openclaw/strategy/recommendations?date=YYYY-MM-DD`
