...
1. 上述命令均须 **exit code 0**；须先执行 `npm run format:changed`，再执行 `lint:changed`、`type-check` 与相关测试（勿与 format 并行）。
...
4. **相关测试必须通过**：凡修改了被测试覆盖的源码（含导出函数/类型/API 路由），须运行对应 `src/__tests__/` 下的测试并全部通过。判定「相关」的依据包括：测试文件 import 了被改模块、测试路径与源码路径对应（见上文「测试文件位置」表）、或本次 diff 触及的符号出现在某测试的 mock/断言中。无对应测试且改动行为非 trivial 时，须补充测试后再汇报完成。
5. 汇报前须确认 format / lint / type-check / 相关测试均已成功。
6. 仅改文档（如 `*.md`）且未触及上述代码路径时，可跳过；若有疑问则仍应运行。
7. 向用户说明本次改动时，须简要写明 format:changed / lint:changed / type-check / 所跑测试均已通过（或说明跳过原因）。
8. **一次性脚本须删除**：本次改动中新建、仅用于当次任务的一次性脚本（`*.js` / `*.mjs` / `*.ts` / `*.py` 等）须在质量门禁全部通过后删除，再向用户汇报完成。`package.json` 引用的常驻 `scripts/` 工具不在此列。
...
This project is indexed by GitNexus as **autobb** (34575 symbols, 63173 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
2. **Run quality gates** (if code changed) — at minimum `npm run format:changed`, `npm run lint:changed`, `npm run type-check`, and targeted `npm test` for affected modules (see「代码修改后的质量门禁」); if SQL/DB touched, also review SQL syntax/aliases/placeholders/types, then `db:migrate`, `validate-schema`, and targeted tests (see「数据库 / SQL 修改后的检查」); add full test suite/build as appropriate
