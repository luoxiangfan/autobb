...
ALTER TABLE launch_scores ADD COLUMN ad_creative_id INTEGER REFERENCES ad_creatives(id) ON DELETE SET NULL;

-- Add issues column to store array of issues (JSON)
ALTER TABLE launch_scores ADD COLUMN issues TEXT;

-- Add suggestions column to store array of suggestions (JSON)
ALTER TABLE launch_scores ADD COLUMN suggestions TEXT;

-- Add content_hash to detect if creative content has changed (for cache invalidation)
ALTER TABLE launch_scores ADD COLUMN content_hash TEXT;

-- Add campaign_config_hash to detect if campaign config has changed
ALTER TABLE launch_scores ADD COLUMN campaign_config_hash TEXT;
