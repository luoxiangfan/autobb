-- ===================================================
-- Migration: 097_ad_creative_types_optimization
-- Description: 广告创意类型优化 - 添加generated_buckets字段和更新prompt v4.16
-- Created: 2025-12-23
-- Version: v4.15 → v4.16
-- Author: Claude Code
-- Safety: 防重复执行 - 使用 INSERT OR IGNORE, PRAGMA table_info 检查
-- Database: SQLite
-- ===================================================
...
CREATE TABLE IF NOT EXISTS _temp_check_col AS
SELECT name FROM pragma_table_info('offers') WHERE name = 'generated_buckets';

INSERT INTO _temp_check_col VALUES ('generated_buckets') ON CONFLICT DO NOTHING;
DROP TABLE IF EXISTS _temp_check_col;
...
-- Step 1.4: 创建索引加速查询
CREATE INDEX IF NOT EXISTS idx_offers_generated_buckets ON offers(generated_buckets);

-- ========================================
-- Part 2: 更新 Prompt v4.15 → v4.16
-- ========================================

-- Step 2.1: 检查是否已是 v4.16 活跃版本（防重复执行）
...
-- Step 2.2: 将当前活跃版本设为非活跃
UPDATE prompt_versions
SET is_active = 0
WHERE prompt_id = 'ad_creative_generation' AND is_active = 1;

-- Step 2.3: 将 v4.16 设为活跃版本
UPDATE prompt_versions
SET is_active = 1
WHERE prompt_id = 'ad_creative_generation' AND version = 'v4.16';
...
-- 验证字段添加成功
SELECT name, sql FROM sqlite_master WHERE type='table' AND name='offers';
