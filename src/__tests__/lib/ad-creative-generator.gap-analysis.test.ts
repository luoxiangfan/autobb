...
import { shouldRunGapAnalysisForCreative } from '@/lib/creatives/generator/test-exports'
...
describe('shouldRunGapAnalysisForCreative', () => {
  ...
  it('stops in coverage mode and ignores defer flag for product-intent D', () => {
    ...
    expect(
      shouldRunGapAnalysisForCreative({ bucket: 'D', deferKeywordSupplementation: true })
    ).toBe(true)
    expect(
      shouldRunGapAnalysisForCreative({ bucket: 'A', deferKeywordSupplementation: true })
    ).toBe(false)
  })
})
