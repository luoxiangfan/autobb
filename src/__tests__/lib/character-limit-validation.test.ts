...
describe('字符限制验证', () => {
  ...
  describe('Sitelinks 长度验证 (text ≤25, desc ≤35)', () => {
    ...
    test('应该正确截断超长 sitelinks', () => {
      const sitelinks = [
        {
          text: 'Compra Ahora en Oferta Especial',
          description: 'Entrega gratuita en 2 días para miembros Prime',
        },
      ]
    ...
    test('应该处理空 sitelinks', () => {
      ...
      const validSitelinks = sitelinks.filter(
        (s) => s && s.text && s.text.length <= 25 && s.description && s.description.length <= 35
      )
  ...
  describe('综合验证', () => {
    test('应该同时验证 callouts、sitelinks 和关键词', () => {
      ...
      // 验证 sitelinks
      const invalidSitelinks = creative.sitelinks.filter(
        (s) => s.text.length > 25 || s.description.length > 35
      )
      expect(invalidSitelinks).toHaveLength(1)
    ...
    test('应该正确处理完全有效的创意', () => {
      const creative = {
        callouts: ['Free Shipping', '免费送货', 'Money Back Guarantee'],
        sitelinks: [
          { text: 'Shop Now', description: 'Free 2-Day Prime Delivery' },
          { text: '立即购买', description: '免费两天送达' },
        ],
        keywords: ['Samsung', 'Samsung Galaxy', 'Samsung Galaxy S24'],
      }
