...
import { clearPromptCache, loadPrompt } from '@/lib/ai/server'
...
describe('prompt-loader fallback behavior', () => {
  ...
  it('loads active prompt when active version exists', async () => {
    mocks.queryOneMock.mockResolvedValueOnce({
    ...
    expect(mocks.queryOneMock).toHaveBeenCalledTimes(1)
    expect(mocks.queryOneMock.mock.calls[0]?.[0]).toContain('is_active = true')
  })

  it('falls back to latest prompt when no active version exists', async () => {
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {})

    mocks.queryOneMock.mockResolvedValueOnce(undefined).mockResolvedValueOnce({
    ...
    expect(mocks.queryOneMock.mock.calls[1]?.[0]).toContain('ORDER BY created_at DESC, id DESC')
  ...
  it('throws when both active and latest prompt are missing', async () => {
    mocks.queryOneMock.mockResolvedValueOnce(undefined).mockResolvedValueOnce(undefined)
