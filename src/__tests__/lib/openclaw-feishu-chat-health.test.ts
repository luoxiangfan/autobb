...
describe('feishu chat health lib', () => {
  ...
  it('lists logs with excerpt and grouped stats', async () => {
    ...
    expect(db.query).toHaveBeenNthCalledWith(3, expect.stringContaining('GROUP BY decision'), [7])
  ...
  it('marks allowed rows as missing when dispatch exceeded threshold without run', async () => {
    ...
    const db = {
      query: vi
        ...
        .mockResolvedValueOnce([])
        .mockResolvedValueOnce([{ decision: 'allowed', total: 1 }])
        .mockResolvedValueOnce([])
        .mockResolvedValueOnce([]),
      exec: vi.fn().mockResolvedValue({ changes: 0 }),
    }
  ...
  it('marks 3-creatives-and-publish workflow as incomplete when D/publish are missing', async () => {
    ...
    const db = {
      query: vi.fn(async (sql: string) => {
        if (
          sql.includes('FROM openclaw_feishu_chat_health_logs') &&
          sql.includes('GROUP BY decision')
        ) {
  ...
  it('keeps workflow running when publish is accepted but campaign is still pending', async () => {
    ...
    const db = {
      query: vi.fn(async (sql: string) => {
        ...
        if (
          sql.includes('parent_request_id IN') ||
          (sql.includes('sender_id IN') && sql.includes('ORDER BY created_at ASC'))
        ) {
          return [
            {
              id: 'run-a',
        ...
        if (sql.includes('FROM creative_tasks')) {
          return [
            ...
            },
            {
              ...
              progress: 100,
              message: 'ok',
              completed_at: '2026-02-10 03:03:30',
              updated_at: '2026-02-10 03:03:30',
            },
          ]
        }
        if (sql.includes('FROM campaigns')) {
