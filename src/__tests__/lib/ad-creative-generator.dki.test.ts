...
import { buildDkiFirstHeadline } from '@/lib/creatives/generator/test-exports'
...
describe('ad-creative-generator DKI', () => {
  it('does not truncate default text for valid DKI length (Google Ads counts defaultText only)', () => {
    ...
    expect(buildDkiFirstHeadline('Armed American Supply')).toBe(
      '{KeyWord:Armed American Supply} Official'
    )
  })

  it('localizes official suffix by target language', () => {
    expect(buildDkiFirstHeadline('Marca', 30, { targetLanguage: 'Spanish' })).toBe(
      '{KeyWord:Marca} Oficial'
    )
    expect(buildDkiFirstHeadline('ブランド', 30, { targetLanguage: 'ja' })).toBe(
      '{KeyWord:ブランド} 公式'
    )
    expect(buildDkiFirstHeadline('品牌', 30, { targetLanguage: 'Chinese' })).toBe(
      '{KeyWord:品牌} 官方'
    )
  })

  it('falls back to country-derived language when target language is missing', () => {
    expect(buildDkiFirstHeadline('Marke', 30, { targetCountry: 'DE' })).toBe(
      '{KeyWord:Marke} Offiziell'
    )
  })
  ...
  it('drops localized suffix when it would exceed max effective length', () => {
    ...
    expect(buildDkiFirstHeadline(brand, 30, { targetLanguage: 'Italian' })).toBe(
      `{KeyWord:${brand}}`
    )
  })
  ...
  it('sanitizes braces from brand to keep valid DKI token', () => {
    expect(buildDkiFirstHeadline('{Brand}', 30, { targetLanguage: 'English' })).toBe(
      '{KeyWord:Brand} Official'
    )
  })
})
