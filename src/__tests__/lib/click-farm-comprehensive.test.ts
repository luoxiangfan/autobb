/**
 * Click Farm 综合集成测试
 * src/__tests__/lib/click-farm-comprehensive.test.ts
 *
 * 测试完整的业务流程和边界条件
 */
...
import {
  ...
  estimateTraffic,
  formatBytes,
} from '@/lib/click-farm/distribution'
...
describe('ClickFarm 综合业务流程测试', () => {
  ...
  const createTestTask = (overrides: Partial<ClickFarmTask> = {}): ClickFarmTask => ({
    ...
    hourly_distribution: [
      0, 0, 0, 0, 0, 5, 10, 15, 12, 10, 8, 5, 5, 8, 10, 12, 15, 20, 25, 22, 18, 15, 10, 5,
    ],
    status: 'running',
    pause_reason: null,
    pause_message: null,
  ...
  describe('业务流程: 从任务创建到完成的完整流程', () => {
    ...
    it('应该正确计算任务的首次执行时间', () => {
      ...
      const task = createTestTask({
        ...
        started_at: null,
        hourly_distribution: [
          0, 0, 0, 0, 0, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10,
        ],
        start_time: '06:00',
      })
  ...
  describe('边界条件测试', () => {
    it('应该正确处理0点击的小时', () => {
      const distribution = [
        0, 0, 0, 0, 0, 0, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10,
      ]
  ...
  describe('历史记录测试', () => {
    ...
    it('应该正确处理零点击的任务', () => {
      ...
      const successRate =
        newTask.total_clicks > 0 ? (newTask.success_clicks / newTask.total_clicks) * 100 : 0
