...
let mockDb: any
let setTaskError: typeof import('@/lib/url-swap/url-swap-task-lifecycle').setTaskError

vi.mock('@/lib/db', () => ({
  getDatabase: () => mockDb,
}))

vi.mock('@/lib/url-swap/alerts/urgent-alerts', () => ({
...
describe('setTaskError', () => {
  ...
  beforeEach(async () => {
    vi.resetModules()
    ;({ setTaskError } = await import('@/lib/url-swap/url-swap-task-lifecycle'))
  }, 30_000)
