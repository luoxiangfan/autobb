...
describe('validateProxyUrl', () => {
  it('accepts IE (Ireland) cc code', () => {
    const url =
      'https://api.iprocket.io/api?username=user&password=pass&cc=IE&ips=1&proxyType=http&responseType=txt'
  ...
  it('accepts NZ (New Zealand) cc code', () => {
    const url =
      'https://api.iprocket.io/api?username=user&password=pass&cc=NZ&ips=1&proxyType=http&responseType=txt'
    const result = validateProxyUrl(url)
    expect(result.isValid).toBe(true)
