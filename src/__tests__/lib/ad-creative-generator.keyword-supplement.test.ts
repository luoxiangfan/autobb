...
} from '@/lib/creatives/generator/index'

const { getKeywordPoolByOfferIdMock } = vi.hoisted(() => ({
  getKeywordPoolByOfferIdMock: vi.fn(),
}))

vi.mock('@/lib/keywords/offer-pool', async () => {
  const actual = await vi.importActual<typeof import('@/lib/keywords/offer-pool')>(
    '@/lib/keywords/offer-pool'
  )
  return {
    ...actual,
