      ...
      try {
        ...
        try {
          ...
          const pageStatus = await page.evaluate(() => {
            ...
            return {
              hasNoJsClass:
                html.classList.contains('a-no-js') || document.body?.classList.contains('a-no-js'),
            ...
            try {
              ...
              if (finalPageStatus.hasJsClass || !finalPageStatus.hasNoJsClass) {
                ...
                if (expectedLangCode !== actualLang) {
                  console.warn(
                    `⚠️ 语言不匹配: 目标国家${options.targetCountry}期望语言${expectedLangCode}，但页面lang=${finalPageStatus.htmlLang}`
                  )
                  console.warn(
        ...
        if (options.waitForSelector) {
          ...
          if (!selectorFound) {
            ...
            try {
              const debugInfo = await page.evaluate(() => {
                ...
                const spans = Array.from(
                  document.querySelectorAll('span[id*="title"], span[class*="title"]')
                )
                  .slice(0, 5)
                  .map((el) => ({
                    tag: 'span',
            ...
            const htmlPreview = await page
              .content()
              .then((html) => html.substring(0, 500))
              .catch(() => '')
