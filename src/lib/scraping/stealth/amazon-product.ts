...
export async function scrapeAmazonProduct(
  ...
  for (let proxyAttempt = 0; proxyAttempt <= maxProxyRetries; proxyAttempt++) {
    try {
      ...
      lastError = error
      console.error(
        `❌ 抓取尝试 ${proxyAttempt + 1}/${maxProxyRetries + 1} 失败: ${error.message?.substring(0, 100)}`
      )
...
export async function scrapeAmazonProductWithContext(
  ...
  try {
    ...
    const scrollResult = await page
      .evaluate(() => {
        const featureBullets = document.querySelector(
          '#feature-bullets, #featurebullets_feature_div'
        )
        if (featureBullets) {
    ...
    if (!skipCompetitorExtraction) {
      ...
      // 检查是否有竞品推荐区域加载
      const hasCompetitorSections = await page
        .evaluate(() => {
          const selectors = [
 ...
 */
function parseAmazonProductHtml(
  $: any,
  url: string,
  ...
  const isInRecommendationArea = (el: any): boolean => {
    ...
    for (let i = 0; i < maxDepth; i++) {
      ...
      if (
        ...
        id.includes('also-viewed') ||
        id.includes('also-bought') ||
        className.includes('sims') ||
        className.includes('related') ||
        className.includes('sponsored') ||
        className.includes('also-viewed') ||
        className.includes('also-bought')
      ) {
        return true
      }
    }
  ...
  const ratingText =
    ...
    // === 移动版选择器 (a-m-* 页面) ===
    $('[data-hook="cr-state-object"]')
      .attr('data-state')
      ?.match(/"averageStarRating":([\d.]+)/)?.[1] ||
  ...
  const amazonChoiceBadge =
    $('.ac-badge-wrapper .ac-badge-text-primary').text().trim() ||
  ...
  const bestSellerBadge =
    $('#zeitgeist-module .a-badge-text').text().trim() ||
  ...
  const availability = (() => {
    ...
    if (
      directSpan &&
      directSpan.length > 5 &&
      directSpan.length < 200 &&
      !directSpan.includes('function')
    ) {
      return directSpan
    }
  ...
  $('p[data-hook="review-collapsed"], span[data-hook="review-body"]')
    .slice(0, 3)
  ...
  $('[data-hook="lighthut-term"], .cr-lighthouse-term, [data-hook="review-filter-tag"]').each(
    (_i: number, el: any) => {
  ...
  if (reviewKeywords.length === 0) {
    $(
      '.cr-vote-buttons + span, [data-hook="review-filter"], .a-declarative[data-action="reviews:filter"]'
  ...
  if (categoryParts.length === 0) {
    $('[data-feature-name="wayfinding-breadcrumbs"] a, .a-breadcrumb a').each(
      (i: number, el: any) => {
        const text = $(el).text().trim()
        if (text) categoryParts.push(text)
      }
      ...
      $('[data-asin]')
        ...
        .each((_i: number, el: any) => {
          ...
          if (
            cleanedAsin &&
            cleanedAsin !== asin &&
            !relatedAsins.includes(cleanedAsin) &&
            !excludedAsins.has(dataAsin || '')
          ) {
            relatedAsins.push(cleanedAsin)
          }
        })
  ...
  const installmentContextPattern =
    /(\/\s*(mo|month)\b|per\s*month|monthly|installment|affirm|klarna|afterpay|payment plan)/i
  ...
  const currentPrice =
    priceCandidates.find((c) => !c.isInstallment)?.value || priceCandidates[0]?.value || null
  ...
  if (!productName) {
    const rawDocTitle =
      $('meta[name="title"]').attr('content')?.trim() || $('title').first().text().trim() || ''
  ...
  const jsonLdPrice = jsonLdData?.price
    ? jsonLdData.currency
      ? `${jsonLdData.currency} ${jsonLdData.price}`
      : jsonLdData.price
    : null
  ...
  if (selectedDomCandidate?.isInstallment && jsonLdPrice) {
    ...
    if (deviationRatio > 0.35) {
      ...
      console.warn(
        `⚠️ DOM价格与JSON-LD价格偏差过大 (${deviationPercent}%)，回退到JSON-LD价格。DOM=${currentPrice}, JSON-LD=${jsonLdPrice}`
      )
      finalPrice = jsonLdPrice
    }
  }
  ...
  if (selectedDomCandidate?.isInstallment && jsonLdData?.price)
...
function extractBrandName(
  ...
  if (productName) {
    ...
    if (titleParts.length > 0) {
      ...
      if (potentialBrand.length >= 2 && potentialBrand.length <= 20) {
        const isValidBrand =
          /^[A-Za-z][A-Za-z0-9&\s-]*$/.test(potentialBrand) || /^[A-Z0-9]+$/.test(potentialBrand)
        if (isValidBrand) {
  ...
  if (affiliateDomainMatch && affiliateDomainMatch[1]) {
    const domainBrand = affiliateDomainMatch[1].replace(/seller|shop|store|amazon/i, '').trim()
  ...
  if (productDescription) {
    const brandMatch = productDescription.match(
      /\b(Brand|Marke|Marque|Marca|Merk)\s*[:：]\s*([A-Z][A-Za-z0-9&\-\s]+)/i
    )
  ...
  const brandScores = new Map<
    string,
    { originalValue: string; totalScore: number; sources: string[] }
  >()
  ...
  let bestBrand: {
    normalized: string
    data: { originalValue: string; totalScore: number; sources: string[] }
  } | null = null
