 ...
 * 通用代理 Axios 客户端
 *
 * 统一的 axios + HttpsProxyAgent 方案，用于需要代理的业务场景：
 * 网页爬取 (scraper.ts)、URL 解析 (url-resolver-enhanced/http/playwright)、
 * 链接检测、Google 搜索建议等需要真实地理位置访问的场景。
 */
...
export async function createProxyAxiosClient(options?: {
  ...
  try {
    ...
    throw new Error(`代理 axios 客户端配置失败: ${error.message}。请检查代理配置。`)
  }
}
