 ...
 * 直接从URL解析，无需API调用
 */
export class CliproxyProvider implements ProxyProvider {
  name = 'Cliproxy'

  canHandle(url: string): boolean {
    const cleanUrl = url.replace(/^https?:\/\//, '')
    if (cleanUrl.includes('@')) return false
    return cleanUrl.includes('cliproxy.io') && cleanUrl.includes(':')
  }
  ...
  validate(url: string): ValidationResult {
    ...
    try {
      ...
      if (!host.includes('cliproxy.io')) {
        errors.push('主机名必须包含 cliproxy.io')
      }
  ...
  async extractCredentials(url: string): Promise<ProxyCredentials> {
    ...
    if (!validation.isValid) {
      throw new Error(`Cliproxy URL验证失败:\n${validation.errors.join('\n')}`)
    }
