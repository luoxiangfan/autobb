  ...
  for (const queue of getQueueManagers()) {
    try {
      ...
      for (const pendingTask of pendingTasks) {
        ...
        const urlSwapTaskId = extractUrlSwapTaskId(pendingTask)
        if (!urlSwapTaskId || !normalizedIds.has(urlSwapTaskId)) continue

        const removed = await queue.removeTask(pendingTask.id)
        if (removed) removedCount += 1
      }
    } catch (error: any) {
      console.warn('[url-swap] 清理队列任务失败:', error?.message || error)
    }
  }
