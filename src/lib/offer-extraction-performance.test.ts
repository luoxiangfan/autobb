...
describe('offer-extraction-performance', () => {
  ...
  it('detects sufficient extracted reviews', () => {
    expect(hasSufficientExtractedReviewsForAnalysis({ topReviews: ['a', 'b', 'c'] }, 'fast')).toBe(true)
    expect(hasSufficientExtractedReviewsForAnalysis({ topReviews: ['a'] }, 'fast')).toBe(false)
  })

  it('mode-specific AI scrape gates', () => {
    expect(shouldRunPlaywrightReviewDeepScrape({ topReviews: ['a', 'b', 'c'] }, true, 'fast')).toBe(false)
    ...
    expect(shouldRunPlaywrightCompetitorDeepScrape({ relatedAsins: [] }, true, 'original')).toBe(true)
  ...
  it('skips independent playwright when baseline fields exist in fast mode', () => {
    ...
    expect(shouldFallbackToRenderedIndependentProductForOffer(baseline, 'https://shop.com/products/widget')).toBe(false)
  })
})
