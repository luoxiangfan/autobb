...
const ECOMMERCE_FULL_DAY_WEIGHTS = [
  ...
  6,
  7,
  8, // 06:00-11:59 早晨逐渐上升
  9,
  9,
  7,
  6,
  5,
  6, // 12:00-17:59 午休小高峰 + 下午工作时段
  8,
  10,
  14,
  15,
  13,
  10, // 18:00-23:59 晚间黄金时段（20-21点最高峰）
]
 ...
 * @returns 归一化后的分布
 */
export function normalizeDistribution(distribution: number[], targetTotal: number): number[] {
...
export function generateDefaultDistribution(
  ...
  // 根据时间段选择对应的曲线
  const weights = startTime === '00:00' ? ECOMMERCE_FULL_DAY_WEIGHTS : ECOMMERCE_DAYTIME_WEIGHTS
