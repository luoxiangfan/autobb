...
async function ensureClickFarmQueueAvailable(queueManager: UnifiedQueueManager): Promise<boolean> {
  ...
  const enforceRedis =
    process.env.NODE_ENV === 'production' && CLICK_FARM_REQUIRE_REDIS_IN_PRODUCTION
...
function getSafeHourlyClickCount(params: {
  ...
  if (normalized !== countNum) {
    console.warn(
      `[Trigger] 任务 ${taskId} 小时${hour}点击数异常(${countNum})，已限制为 ${normalized}`
    )
  }
...
export function getRemainingHourlyClicks(
  ...
  const cappedPlanned = target && target > 0 ? Math.min(planned, Math.floor(target)) : planned
...
function buildClickFarmBatchTaskId(data: ClickFarmBatchTaskData): string {
  const hour = String(Math.min(23, Math.max(0, Math.floor(Number(data.targetHour) || 0)))).padStart(
    2,
    '0'
  )
...
export async function triggerTaskScheduling(
  ...
  if (!proxyConfig) {
    await pauseClickFarmTask(task.id, 'no_proxy', `缺少${offer.target_country}国家的代理配置`)
