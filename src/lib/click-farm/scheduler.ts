...
export function calculateProgress(task: ClickFarmTask): number {
  ...
  // 计算完整日期差
  const elapsedDays = Math.floor((nowDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24))

  const progress = Math.min(100, Math.round((elapsedDays / task.duration_days) * 100))
...
function getNextHourInTimezone(now: Date, timezone: string): Date {
  ...
  // 在任务时区中构造下一个整点的时间
  return createDateInTimezone(nextDate, `${nextHour.toString().padStart(2, '0')}:00`, timezone)
}
...
function getNextHourWithQuota(now: Date, task: ClickFarmTask): Date {
  ...
  for (let i = 1; i <= 24; i++) {
    ...
    const checkDate = currentHour + i >= 24 ? incrementDate(currentDate) : currentDate
