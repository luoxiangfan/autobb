...
import { getLoginCustomerId, AuthType } from '@/lib/google-ads/service-account/service-account'
import {
...
export async function getKeywordIdeas(params: {
  ...
  try {
    ...
    googleAdsKeywordLogger.debug('seed_batches_prepared', {
      seedCount: params.seedKeywords?.length || 0,
    ...
    for (let batchIndex = 0; batchIndex < seedBatches.length; batchIndex++) {
      ...
      googleAdsKeywordLogger.debug('seed_batch_completed', {
        batchIndex: batchIndex + 1,
        batchTotal: seedBatches.length,
        batchIdeas: batchIdeas.length,
        cumulativeIdeas: allKeywordIdeas.length,
      })
    }
    ...
    googleAdsKeywordLogger.error('get_keyword_ideas_failed', {}, error)
...
export async function getKeywordMetrics(params: {
  ...
  try {
    ...
    success = false
    errorMessage = error.message
    googleAdsKeywordLogger.error('get_keyword_metrics_failed', {}, error)
