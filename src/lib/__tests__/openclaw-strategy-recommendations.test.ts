...
import { __testUtils } from '@/lib/openclaw/strategy/strategy-recommendations'
...
describe('openclaw strategy recommendations rules', () => {
  ...
  it('includes offline recommendation for lower-value duplicate campaigns of same offer', () => {
    ...
    const duplicateOfflineRec = drafts.find(
      (item) => item.campaignId === 202 && item.recommendationType === 'offline_campaign'
    )
    expect(duplicateOfflineRec).toBeTruthy()
    expect(duplicateOfflineRec?.data.ruleCode).toBe('offline_duplicate_offer_campaign')

    const winnerOfflineRec = drafts.find(
      (item) => item.campaignId === 201 && item.recommendationType === 'offline_campaign'
    )
  ...
  it('builds keyword expansion plan with explicit match types', () => {
    ...
    expect(
      (expandRec?.data.keywordPlan || []).every((kw) =>
        ['BROAD', 'PHRASE', 'EXACT'].includes(kw.matchType)
      )
    ).toBe(true)
    expect(
      (expandRec?.data.keywordPlan || []).every((kw) => kw.text.toLowerCase().includes('dreo'))
    ).toBe(true)
    expect((expandRec?.data.keywordPlan || []).every((kw) => Boolean(kw.whySelected))).toBe(true)
    expect(
      (expandRec?.data.keywordPlan || []).every(
        (kw) => typeof kw.evidenceMetrics?.impressions === 'number'
      )
    ).toBe(true)
    expect(
      (expandRec?.data.keywordPlan || []).every(
        (kw) => kw.conflictCheck?.negativeConflict === false
      )
    ).toBe(true)
    expect(
      (expandRec?.data.keywordPlan || []).every(
        (kw) => kw.conflictCheck?.duplicateConflict === false
      )
    ).toBe(true)
    expect(
      (expandRec?.data.keywordPlan || []).every((kw) => kw.sourceLayer === 'recent_search_terms')
    ).toBe(true)
    expect(
      (expandRec?.data.keywordPlan || []).every((kw) => Number.isFinite(Number(kw.selectionScore)))
    ).toBe(true)
    expect(expandRec?.data.keywordPlanDiagnostics?.candidateCountRecent).toBe(searchTerms.length)
    expect(expandRec?.data.keywordPlanDiagnostics?.selectedFromRecent).toBe(
      (expandRec?.data.keywordPlan || []).length
    )
  })
  ...
  it('uses historical search terms when recent pool is empty', () => {
    ...
    expect(
      (expandRec?.data.keywordPlan || []).every(
        (kw) => kw.sourceLayer === 'historical_search_terms'
      )
    ).toBe(true)
    expect(expandRec?.data.keywordPlanDiagnostics?.candidateCountRecent).toBe(0)
    expect(expandRec?.data.keywordPlanDiagnostics?.candidateCountHistorical).toBe(
      historicalTerms.length
    )
    expect(expandRec?.data.keywordPlanDiagnostics?.selectedFromHistorical).toBe(
      (expandRec?.data.keywordPlan || []).length
    )
  })
  ...
  it('keeps recent priority and records excluded reasons in diagnostics', () => {
    const recentTerms = [
      {
        searchTerm: 'dreo portable ac pro',
        impressions: 140,
        clicks: 18,
        conversions: 2,
        cost: 12,
      ...
      {
        searchTerm: 'amazon dreo portable ac',
        impressions: 130,
        clicks: 15,
        conversions: 0,
        cost: 11,
    ...
    expect(
      (expandRec?.data.keywordPlan || []).some((kw) => kw.sourceLayer === 'historical_search_terms')
    ).toBe(true)
  ...
  it('filters expansion keywords that conflict with campaign negative keywords', () => {
    const searchTerms = [
      {
        searchTerm: 'renpho digital body scales',
        impressions: 95,
        clicks: 8,
        conversions: 0,
        cost: 6,
      ...
      {
        searchTerm: 'renpho body scale review',
        impressions: 90,
        clicks: 7,
        conversions: 0,
        cost: 5.5,
      ...
      {
        searchTerm: 'renpho body scale discount',
        impressions: 88,
        clicks: 7,
        conversions: 0,
        cost: 5.3,
      ...
      {
        searchTerm: 'renpho smart body scale',
        impressions: 84,
        clicks: 6,
        conversions: 1,
        cost: 4.8,
      ...
      {
        searchTerm: 'renpho bluetooth body scale',
        impressions: 80,
        clicks: 6,
        conversions: 0,
        cost: 4.6,
  ...
  it('generates negative keyword recommendations from hard-negative search terms', () => {
    const drafts = __testUtils.buildRecommendationDrafts({
      ...
      keywordInventoryByCampaign: new Map([
        [109, [{ text: 'dreo', matchType: 'EXACT', isNegative: false }]],
      ]),
      searchTermsByCampaign: new Map([
        [
          109,
          [
            {
              searchTerm: 'dreo manual download',
              impressions: 50,
              clicks: 6,
              conversions: 0,
              cost: 5.8,
    ...
    expect(
      (negativeRec?.data.negativeKeywordPlan || []).every((kw) => kw.matchType === 'EXACT')
    ).toBe(true)
  })
  ...
  it('generates match type optimization recommendation for broad keywords with cost', () => {
    const drafts = __testUtils.buildRecommendationDrafts({
      ...
      keywordInventoryByCampaign: new Map([
        [110, [{ text: 'dreo', matchType: 'BROAD', isNegative: false }]],
      ]),
      searchTermsByCampaign: new Map([
        [110, [{ searchTerm: 'dreo', impressions: 200, clicks: 26, conversions: 0, cost: 14.2 }]],
      ]),
      creativeById: new Map(),
    })
  ...
  it('treats keyword execution partial failures as failure', () => {
    expect(() =>
      __testUtils.assertRecommendationActionResult({
  ...
  it('detects already-offline API errors as idempotent', () => {
    ...
    expect(
      __testUtils.isAlreadyOfflineCampaignError(
        new Error('AutoAds API error (400): {"error":"其他错误"}')
      )
    ).toBe(false)
  })
  ...
  it('normalizes report_date values returned as Date/timestamp into YYYY-MM-DD', () => {
    ...
    expect(__testUtils.normalizeRecommendationReportDate('2026-02-25T13:45:00.000Z')).toBe(
      '2026-02-25'
    )
  })
})
