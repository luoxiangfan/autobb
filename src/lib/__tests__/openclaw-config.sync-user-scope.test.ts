...
const { getSettingsByCategoryMock, getOpenclawGatewayTokenMock, collectUserFeishuAccountsMock } =
  vi.hoisted(() => ({
...
describe('syncOpenclawConfig user scope', () => {
  ...
  it('uses global AI settings during actor user sync', async () => {
    ...
    getSettingsByCategoryMock
      .mockResolvedValueOnce([{ key: 'ai_models_json', value: userAiModelsJson }])
      .mockResolvedValueOnce([{ key: 'ai_models_json', value: globalAiModelsJson }])
  ...
  it('keeps user Feishu account credentials from existing config when user aggregate is empty', async () => {
    ...
    getSettingsByCategoryMock.mockResolvedValueOnce([]).mockResolvedValueOnce([])
