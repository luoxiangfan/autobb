...
import { canonicalizeOfferBatchCsvHeader, decodeCsvTextSmart } from '@/lib/offers/server'
...
describe('offer batch csv header', () => {
  it('supports Chinese headers (with BOM)', () => {
    ...
    expect(canonicalizeOfferBatchCsvHeader('单品推广链接4')).toBe('product_link_4')
    expect(canonicalizeOfferBatchCsvHeader('单品推广链接5')).toBe('product_link_5')
    expect(canonicalizeOfferBatchCsvHeader('单品推广链接6')).toBe('product_link_6')
