...
describe('collectUserFeishuAccounts', () => {
  ...
  it('merges card settings from user feishu_accounts_json.main', async () => {
    ...
    expect(accounts['user-4'].cardConfirmUrl).toBe(
      'https://example.com/api/openclaw/commands/confirm'
    )
  ...
  it('collects binding accounts without decrypting sensitive fields', async () => {
    hoisted.queryMock.mockResolvedValueOnce([
      ...
      },
      {
        user_id: 6,
        key: 'feishu_target',
        value: 'feishu:ou_target_6',
        encrypted_value: null,
        is_sensitive: false,
      },
      {
        user_id: 6,
        key: 'feishu_auth_mode',
        value: 'strict',
        encrypted_value: null,
        is_sensitive: false,
      },
