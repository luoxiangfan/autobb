...
describe('sendDailyReportToFeishu', () => {
  ...
  it('coalesces concurrent delivery calls for same delivery task id', async () => {
    ...
    await Promise.all([sendDailyReportToFeishu(params), sendDailyReportToFeishu(params)])
  ...
  it('formats daily report message with aligned daily metrics and clear conversion labels', async () => {
    ...
    expect(message).toContain(
      '- 预算概览（启用中Campaign）：预算 123.33 USD｜已花费 41.21 USD｜剩余 82.12 USD'
    )
  ...
  it('includes all currencies in spend and budget sections when budget payload is mixed-currency', async () => {
    ...
    expect(message).toContain(
      '- 预算概览（启用中Campaign，多币种）：CNY：预算 130 CNY｜已花费 0 CNY｜剩余 130 CNY；USD：预算 75 USD｜已花费 15 USD｜剩余 60 USD'
    )
  })
  ...
  it('expands cached mixed-currency roi summary by currency', async () => {
    ...
    hoisted.fetchAutoadsJsonMock.mockImplementation(
    ...
    expect(message).toContain(
      '- ROI概览（多币种）：CNY：佣金 0 CNY｜花费 0 CNY｜利润 0 CNY；USD：佣金 25 USD｜花费 19 USD｜利润 6 USD'
    )
    expect(message).toContain(
      '- 回报率（多币种）：CNY：ROAS 暂不可用｜ROI 暂不可用；USD：ROAS 1.32x｜ROI 31.58%'
    )
    expect(message).toContain(
      '- 预算概览（启用中Campaign，多币种）：CNY：预算 130 CNY｜已花费 0 CNY｜剩余 130 CNY；USD：预算 75 USD｜已花费 15 USD｜剩余 60 USD'
    )
  ...
  it('includes top strategy recommendations in daily report message', async () => {
            ...
            strategyRecommendations: [
              {
                ...
                campaignId: 11,
                data: {
                  campaignName: 'Dovoh_3679',
                  impactConfidenceReason: '样本：曝光 1000 / 点击 88 / 花费 21.00 / ROAS 0.00',
                },
              },
              {
                id: 'r2',
    ...
    const message = String(hoisted.invokeOpenclawToolMock.mock.calls[0]?.[0]?.args?.message || '')
    expect(message).toContain(
      '建议状态：总 3｜待执行 2｜已执行 0｜执行失败 0｜待重算 0｜暂不执行 1'
    )
              ...
              data: {
                overall: {
                  ...
                  affiliateReconciliation: {
                    ...
                    topFailureReasons: [
                      {
                        code: 'product_mapping_miss',
                        label: '商品映射缺失',
                        count: 3,
                        commission: 3,
                      },
                      {
                        code: 'campaign_mapping_miss',
                        label: '无活动Campaign',
                        count: 1,
                        commission: 1,
                      },
