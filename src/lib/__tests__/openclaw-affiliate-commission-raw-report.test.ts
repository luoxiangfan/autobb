...
describe('affiliate-commission-raw-report', () => {
  ...
  it('aggregates yeahpromos rows by advert_id and partnerboost rows by asin+brand', async () => {
    ...
    hoisted.dbQueryMock
      ...
      .mockResolvedValueOnce([{ user_id: 1, asin: 'B0BGPF71Q6', brand: 'Example Brand' }])
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([])
  ...
  it('returns date detail grouped by brand', async () => {
    hoisted.dbQueryMock
      ...
      .mockResolvedValueOnce([{ user_id: 1, asin: 'B0TEST1234', brand: 'Brand B' }])
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([])
  ...
  it('validates requested admin user ids without loading all users', async () => {
    hoisted.dbQueryMock.mockResolvedValueOnce([{ id: 2 }, { id: 3 }])
  ...
  it('detects ASIN presence in offer url or final_url', () => {
    expect(offerUrlsContainAsin('https://www.amazon.com/dp/B0BGPF71Q6', null, 'B0BGPF71Q6')).toBe(
      true
    )
    expect(
      offerUrlsContainAsin(null, 'https://example.com/product?asin=B0TEST1234', 'B0TEST1234')
    ).toBe(true)
    expect(
      offerUrlsContainAsin(
  ...
  it('falls back to global affiliate product pool when user products are missing', async () => {
    ...
    hoisted.dbQueryMock
      ...
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([{ asin: 'B0GLOBAL01', brand: 'Global Pool Brand' }])
      ...
      .mockResolvedValueOnce([
        {
          ...
          response_payload: JSON.stringify({
            data: {
              list: [{ sale_comm: 20, asin: 'B0BGPF71Q6', order_id: 'order-1' }],
            },
          }),
        },
        {
