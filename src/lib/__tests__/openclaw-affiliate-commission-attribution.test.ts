...
describe('persistAffiliateCommissionAttributions simplified attribution', () => {
  ...
  it('rebuilds historical platform snapshot when fetched commission is lower', async () => {
        ...
        return [
          {
            id: 2401,
            brand: 'Novilla',
            url: 'https://www.amazon.com/dp/B0HISTLOW1',
            final_url: null,
            affiliate_link: null,
          },
        ]
    ...
    expect(deleteCalls[0]?.[1]).toEqual([7, '2000-01-01', 'partnerboost'])
    ...
    expect((insertCalls[0]?.[1] as any[])[8]).toBe(5)
  })

  it('clears historical platform snapshot when queried platform returns no commission rows', async () => {
    const result = await persistAffiliateCommissionAttributions({
      userId: 7,
      reportDate: '2000-01-01',
      entries: [],
    ...
    expect(deleteCalls).toHaveLength(1)
    expect(deleteCalls[0]?.[1]).toEqual([7, '2000-01-01', 'yeahpromos'])
  ...
  it('splits explicit ASIN commission by rolling cost share', async () => {
    ...
    const attributionInsertCalls = exec.mock.calls.filter(
      ([sql]) =>
        typeof sql === 'string' && sql.includes('INSERT INTO affiliate_commission_attributions')
    )
        ...
        return [
          {
            id: 2101,
            brand: 'Novilla',
            url: 'https://www.amazon.com/dp/B0CLICKS01',
            final_url: null,
            affiliate_link: null,
          },
          {
        ...
        return [
          {
            id: 2201,
            brand: 'Novilla',
            url: 'https://www.amazon.com/dp/B0KNOWN001',
            final_url: null,
            affiliate_link: null,
          },
          {
            id: 2202,
            brand: 'Novilla',
            url: 'https://www.amazon.com/dp/B0KNOWN002',
            final_url: null,
            affiliate_link: null,
          },
        ]
  ...
  it('keeps previously attributed events frozen', async () => {
    ...
    query.mockImplementation(async (sql: string) => {
      if (sql.includes('FROM affiliate_commission_attributions') && sql.includes('AS event_id')) {
        return [
          {
            event_id: 'yeahpromos|evt-frozen-1',
            offer_id: 2301,
            campaign_id: 3301,
            commission_amount: 6.66,
          },
        ]
      }
      if (
  ...
  it('continues attribution when existing-event lookup times out', async () => {
    const today = formatLocalYmd(new Date())
        ...
        return [
          {
            id: 2301,
            brand: 'Novilla',
            url: 'https://www.amazon.com/dp/B0TIMEOUT1',
            final_url: null,
            affiliate_link: null,
          },
        ]
    ...
    const eventLookupCall = query.mock.calls.find(
      ([sql]) =>
        typeof sql === 'string' &&
        sql.includes('FROM affiliate_commission_attributions') &&
        sql.includes('AS event_id')
    )
    expect(eventLookupCall).toBeTruthy()
    expect((eventLookupCall?.[1] as any[])[1]).toBe(today)
  })
  ...
  it('marks commission unattributed when neither ASIN nor brand can map to campaigns', async () => {
    ...
    const failureInsertCalls = exec.mock.calls.filter(
      ([sql]) =>
        typeof sql === 'string' &&
        sql.includes('INSERT INTO openclaw_affiliate_attribution_failures')
    )
        ...
        return [
          {
            id: 4001,
            brand: 'Waterdrop',
            url: 'https://www.amazon.com/dp/B0LINKED01',
            final_url: null,
            affiliate_link: null,
          },
        ]
  ...
  it('uses linked affiliate product brand as campaign brand alias for fallback attribution', async () => {
    ...
    query.mockImplementation(async (sql: string) => {
        ...
        return [
          {
            id: 6194,
            brand: 'Max & Lily',
            url: 'https://www.amazon.com/dp/B0BLTB51D7',
            final_url: null,
            affiliate_link: null,
          },
        ]
      ...
      if (sql.includes('FROM affiliate_product_offer_links apol')) {
        return [{ offer_id: 6194, asin: 'B0BLTB51D7', brand: "Max & Lily Kid's Furniture" }]
      }
      if (sql.includes('FROM affiliate_products') && sql.includes('brand IS NOT NULL')) {
        return [{ asin: 'B08KTR8FJJ', brand: "Max & Lily Kid's Furniture" }]
      }
      if (sql.includes('FROM campaigns c')) {
        return [
          {
        ...
        return [
          {
            id: 6001,
            brand: 'Reolink',
            url: 'https://www.amazon.com/dp/B0REOLINK1',
            final_url: null,
            affiliate_link: null,
          },
          {
            id: 6002,
            brand: 'Roborock',
            url: 'https://www.amazon.com/dp/B0ROBOROCK1',
            final_url: null,
            affiliate_link: null,
          },
        ]
        ...
        return [
          {
            id: 6004,
            brand: 'Lefant',
            url: 'https://www.amazon.com/dp/B0LEFANT01',
            final_url: null,
            affiliate_link: null,
          },
        ]
        ...
        return [
          {
            id: 6003,
            brand: 'Livfresh',
            url: 'https://www.amazon.com/dp/B0LIVFRESH1',
            final_url: null,
            affiliate_link: null,
          },
        ]
        ...
        return [
          {
            id: 6101,
            brand: 'Novilla',
            url: 'https://www.amazon.com/dp/B0REMOVED1',
            final_url: null,
            affiliate_link: null,
          },
        ]
          ...
          return [
            {
              id: 6201,
              brand: 'Dormify',
              url: 'https://www.amazon.com/dp/B0REMOVED2',
              final_url: null,
              affiliate_link: null,
            },
          ]
        ...
        return [
          {
            id: 6004,
            brand: 'Waterdrop',
            url: 'https://www.amazon.com/dp/B0WATERDROP1',
            final_url: null,
            affiliate_link: null,
          },
          {
            ...
            url: 'https://www.amazon.com/dp/B0WAHL1',
            final_url: null,
            affiliate_link: null,
          },
        ]
