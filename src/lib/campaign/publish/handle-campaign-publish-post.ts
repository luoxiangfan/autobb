...
import {
  SINGLE_BRAND_PER_ACCOUNT_ENFORCED,
  extractAllIssues,
  extractAllSuggestions,
  extractGoogleAdsRequestId,
  isGoogleAdsAccountPermissionDenied,
  isOAuthTokenExpiredOrRevoked,
  isTruthyFlag,
  normalizeAccountIdInput,
  normalizeBrand,
  normalizeCustomerId,
  toSafePositiveInt32,
} from '@/lib/campaign/publish/publish-route-helpers'

export type CampaignPublishAuthUser = {
  userId: number
  role?: string
  email?: string
}

export async function handleCampaignPublishPost(request: Request, user: CampaignPublishAuthUser) {
  try {
    ...
    const _enableCampaignImmediately =
      enableCampaignImmediately ?? enable_campaign_immediately ?? false
    ...
    if (accountIdAsInt32 !== null) {
      ...
      `,
        [accountIdAsInt32, Number(userId), 1]
      )) as any
      if (adsAccount) {
        accountLookupBy = 'id'
      }
    ...
    if (!adsAccount && normalizedCustomerId) {
      adsAccount = (await db.queryOne(
        ...
        LIMIT 1
      `,
        [Number(userId), 1, normalizedCustomerId]
      )) as any
      if (adsAccount) {
        ...
        return NextResponse.json(
          {
            ...
            message:
              '当前Google Ads账号已无访问权限，系统已自动下线该账号。请刷新账号列表后选择可用账号重试。',
            details: {
    ...
    if (SINGLE_BRAND_PER_ACCOUNT_ENFORCED && currentBrandNormalized) {
      const enabledOtherBrandCampaigns = activeCampaignsResult.otherCampaigns.filter((campaign) => {
        const campaignBrand = normalizeBrand(
          (campaign as any).parsedNaming?.brandName || (campaign as any).name
        )
      ...
      if (enabledOtherBrandCampaigns.length > 0 && !_pauseOldCampaigns) {
        const warningMessage =
          '检测到Ads账号中存在其他品牌Campaign，当前发布将继续执行。建议尽快评估并暂停其他品牌Campaign，避免多品牌并行投放。'
      ...
      return NextResponse.json(
        {
          ...
          options: [
            {
              label: '暂停并发布',
              value: 'pause_and_publish',
              description: '推荐：先暂停所有旧广告，再发布新广告',
            },
            {
              label: '直接发布（A/B测试）',
              value: 'publish_together',
              description: '旧广告继续运行，新广告同时激活',
            },
        ...
        return NextResponse.json(
          {
            ...
            message:
              'campaignConfig URL字段与系统归属来源不一致，请移除这些字段或改为与创意/Offer一致',
    ...
    const publishHashCampaignConfig =
      launchScoreHashConfigFromPublishCampaignConfig(_campaignConfig)
    ...
    try {
      if (cachedLaunchScore) {
        ...
        if (
          creativeForLaunchScore.keywordsWithVolume &&
          creativeForLaunchScore.keywordsWithVolume.length > 0
        ) {
          const kwMatchTypeDist = creativeForLaunchScore.keywordsWithVolume.reduce(
            (acc: any, kw: any) => {
              const type = kw.matchType || 'UNKNOWN'
              acc[type] = (acc[type] || 0) + 1
              return acc
    ...
    const resolvedCampaignName =
      baseCampaignName || naming.associativeCampaignName || naming.campaignName
