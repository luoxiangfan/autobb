...
export default function ClickFarmPage() {
  ...
  const filterTasks = useCallback(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(
        (t) => t.id.toLowerCase().includes(query) || t.offer_id.toString().includes(query)
      )
    }
  ...
  const getTodayProgressInfo = (
    task: any
  ): { percent: number; actual: number; target: number } | null => {
    try {
  ...
  const handleTriggerTask = async (taskId: string) => {
    try {
      ...
      const result = await safeJsonParse<{ message?: string; error?: string; success?: boolean }>(
        response
      )
      if (!result.success) {
  ...
  const handleBatchDelete = async () => {
    ...
    try {
      ...
      if (errors.length > 0) {
        // 在对话框内显示错误，不关闭对话框
        setBatchDeleteError(
          `${errors.length}/${selectedTaskIds.size} 个任务删除失败：\n${errors.join('\n')}`
        )
        // 刷新列表以显示成功删除的结果
        await loadData()
        return
  ...
  const selectedSelectableCount = selectablePaginatedTasks.filter((task) =>
        ...
        {stats && (
          <Card className="mb-6">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-gray-600">任务状态分布</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="w-[50px]">
                      <Checkbox
                        checked={
                          selectablePaginatedTasks.length > 0 &&
                          selectedSelectableCount === selectablePaginatedTasks.length
                        }
                    ...
                    <SortableHeader field="offer" className="w-[180px]">
                      Offer
                    </SortableHeader>
                    <SortableHeader field="status" className="w-[100px]">
                      状态
                    </SortableHeader>
                    <SortableHeader field="country" className="w-[108px]">
                      国家/时区
                    </SortableHeader>
                    <SortableHeader field="dailyClicks" className="w-[96px]">
                      每日点击
                    </SortableHeader>
                    <SortableHeader field="progress" className="w-[80px]">
                      进度
                    </SortableHeader>
                    <SortableHeader field="successRate" className="w-[92px]">
                      成功率
                    </SortableHeader>
                    <SortableHeader field="startDate" className="w-[112px]">
                      开始日期
                    </SortableHeader>
                    ...
                    <TableRow key={task.id} className="hover:bg-gray-50/50">
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1">
                          {(() => {
                            const todayInfo =
                              task.duration_days === -1 ? getTodayProgressInfo(task) : null
                            const progressValue =
