...
export default function UserManagementPage() {
  ...
  const [createGoogleAdsAuthSetup, setCreateGoogleAdsAuthSetup] =
    useState<CreateGoogleAdsAuthSetup>('none')
  const [createGoogleAdsAuthType, setCreateGoogleAdsAuthType] = useState<
  ...
  const handleCreateUser = async () => {
    ...
    try {
      ...
      if (createGoogleAdsAuthSetup !== 'none' && newUserId) {
        try {
          await applyGoogleAdsAuthForUser(
            newUserId,
            createGoogleAdsAuthSetup,
            createGoogleAdsAuthType
          )
        } catch (authError: any) {
  ...
  const handleToggleProductManagement = async (
    userId: number,
    username: string,
    currentEnabled: boolean
  ) => {
  ...
  const handleToggleStrategyCenter = async (
    userId: number,
    username: string,
    currentEnabled: boolean
  ) => {
  ...
  const copyToClipboard = () => {
    ...
    const domain =
      window.location.hostname === 'localhost' ? 'http://localhost:3000' : 'https://www.autoads.dev'
...
首次登录需要修改密码。`
  ...
  const openEditModal = (user: User) => {
    ...
    setEditExpiry(
      user.packageExpiresAt ? new Date(user.packageExpiresAt).toISOString().split('T')[0] : ''
    )
            ...
            <Table className="w-max min-w-[1040px] table-fixed text-sm [&_th]:h-9 [&_th]:px-2 [&_td]:px-2 [&_td]:py-2">
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="w-[96px] text-center whitespace-nowrap sm:w-[136px]">
                    操作
                  </TableHead>
                </TableRow>
              </TableHeader>
                    ...
                    <TableRow key={user.id}>
                      ...
                      <TableCell className="min-w-0">
                        <div className="flex min-w-0 items-center gap-1.5">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="hidden truncate text-xs text-muted-foreground 2xl:block">
                              {user.email || '无邮箱'}
                            </div>
                          </div>
                        </div>
                      ...
                      <TableCell className="hidden whitespace-nowrap md:table-cell">
                        ...
                        >
                          {user.packageType === 'trial'
                            ? '试用版'
                            : user.packageType === 'annual'
                              ? '年度会员'
                              : user.packageType === 'lifetime'
                                ? '长期会员'
                                : '私有化部署'}
                        </Badge>
                      </TableCell>
                      <TableCell className="hidden text-muted-foreground whitespace-nowrap lg:table-cell">
                        {user.packageExpiresAt
                          ? new Date(user.packageExpiresAt).toLocaleDateString('zh-CN')
                          : '长期'}
                      </TableCell>
                      ...
                      <TableCell className="hidden text-muted-foreground whitespace-nowrap xl:table-cell">
                        {user.lastLoginAt ? (
                          <span className="text-xs">
                            ...
                            {new Date(user.lastLoginAt).toLocaleTimeString('zh-CN', {
                              hour: '2-digit',
                              minute: '2-digit',
                            })}
                          </span>
                      ...
                      <TableCell className="whitespace-nowrap">
                        <div className="flex flex-col gap-1">
                          ...
                          {!user.isActive ? (
                            ...
                            <Badge
                              variant="outline"
                              className="h-6 px-2 text-xs text-yellow-600 border-yellow-600 whitespace-nowrap"
                            >
                            ...
                            <Badge
                              variant="outline"
                              className="h-6 px-2 text-xs text-green-600 border-green-600 whitespace-nowrap"
                            >
                        ...
                        <div className="flex items-center justify-center gap-0">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-72">
                              ...
                              <DropdownMenuItem
                                onClick={() =>
                                  handleToggleProductManagement(
                                    user.id,
                                    user.username,
                                    user.productManagementEnabled
                                  )
                                }
                                className="items-start gap-2 py-2"
                                title={
                              ...
                              >
                                <Boxes
                                  className={`w-4 h-4 mt-0.5 shrink-0 ${user.productManagementEnabled ? 'text-emerald-600' : 'text-gray-500'}`}
                                />
                                <div>
                              ...
                              <DropdownMenuItem
                                onClick={() =>
                                  handleToggleStrategyCenter(
                                    user.id,
                                    user.username,
                                    user.strategyCenterEnabled
                                  )
                                }
                                className="items-start gap-2 py-2"
                                title={
                              ...
                              >
                                <TrendingUp
                                  className={`w-4 h-4 mt-0.5 shrink-0 ${user.strategyCenterEnabled ? 'text-blue-600' : 'text-gray-500'}`}
                                />
                                <div>
                                  <div className="font-medium">
                                    {user.strategyCenterEnabled
                              ...
                              <DropdownMenuItem
                                onClick={() =>
                                  handleToggleOpenclaw(user.id, user.username, user.openclawEnabled)
                                }
                                className="items-start gap-2 py-2"
                                title={
                              ...
                              >
                                ...
                                <div>
                                  ...
                                  <div className="text-xs text-muted-foreground">
                                    切换该用户的 OpenClaw 使用权限
                                  </div>
                                </div>
                              </DropdownMenuItem>
                              <DropdownMenuItem
                                onClick={() =>
                                  handleDisableUser(user.id, user.username, user.isActive)
                                }
                              ...
                              <DropdownMenuItem
                                onClick={() =>
                                  handleDeleteUser(user.id, user.username, user.isActive)
                                }
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>创建一个新用户账号。默认密码为 auto11@20ads</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              {createUsername && (
                <p className="text-caption text-muted-foreground">
                  生成的用户名:{' '}
                  <span className="font-medium text-foreground">{createUsername}</span>
                </p>
              )}
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <p className="text-caption text-muted-foreground">
                ...
                {(createPackage === 'lifetime' || createPackage === 'enterprise') &&
                  '长期会员/企业版: 2099-12-31'}
              </p>
            </div>
            <div className="space-y-2">
              <Label>
                邮箱地址 <span className="text-caption text-muted-foreground">(可选)</span>
              </Label>
              <Input
            ...
            <div className="space-y-3 rounded-lg border p-4">
              <Label>
                Google Ads 认证 <span className="text-caption text-muted-foreground">(可选)</span>
              </Label>
              <Select
                ...
                <div className="space-y-3 pt-1">
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        setCreateGoogleAdsAuthType(v as 'oauth' | 'service_account')
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
                  ...
                  {createGoogleAdsAuthSetup === 'shared_admin' ? (
                    <p className="text-sm text-muted-foreground bg-muted rounded-md p-3">
                      将使用当前登录管理员的{' '}
                      {createGoogleAdsAuthType === 'oauth' ? 'OAuth' : '服务账号'} 配置。
                    </p>
                    ...
                    <div className="space-y-2">
                      {(
                        ...
                        <div key={key}>
                          ...
                          <Input
                            type={
                              key.includes('secret') || key.includes('token') ? 'password' : 'text'
                            }
                            value={createGoogleAdsOauthForm[key]}
                            onChange={(e) =>
                              setCreateGoogleAdsOauthForm((prev) => ({
                                ...prev,
                                [key]: e.target.value,
                              }))
                            }
                    ...
                    <div className="space-y-2">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setCreateGoogleAdsServiceAccountForm((p) => ({
                              ...p,
                              name: e.target.value,
                            }))
                          }
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!createUsername || !createExpiry || isSubmitting}
            >
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="p-4 bg-muted rounded-lg space-y-2 font-mono text-sm">
              ...
              <div>
                登录密码:{' '}
                <span className="font-bold text-indigo-600">{resetPasswordData?.password}</span>
              </div>
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                          ...
                          <div className="col-span-2">
                            ...
                            <span className="text-xs font-mono text-muted-foreground">
                              ...
                              {record.userAgent.substring(0, 80)}
                          ...
                          {record.details && (
                            <div className="col-span-2">
                              ...
                              <span className="text-xs">{JSON.stringify(record.details)}</span>
                            </div>
                          )}
                        </div>
                      </div>
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-foreground mb-3">{alert.description}</div>
