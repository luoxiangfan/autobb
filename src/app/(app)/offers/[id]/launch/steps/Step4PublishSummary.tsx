...
export default function Step4PublishSummary({
  ...
  const [accountPublishResults, setAccountPublishResults] = useState<
    Array<{
      accountId: number
      customerId: string
      accountName: string
  ...
  const getCalloutText = (callout: any): string => {
    ...
    if (callout && typeof callout === 'object' && typeof callout.text === 'string')
      return callout.text
    return ''
  }
  ...
  const toCalloutTextArray = (value: any): string[] => {
    if (!Array.isArray(value)) return []
    return value.map((c: any) => getCalloutText(c).trim()).filter((c: string) => c.length > 0)
  }
  ...
  const effectiveHeadlines =
    toNonEmptyStringArray(campaignConfig?.headlines).length > 0
  ...
  const effectiveDescriptions =
    toNonEmptyStringArray(campaignConfig?.descriptions).length > 0
      ? toNonEmptyStringArray(campaignConfig?.descriptions)
      : toNonEmptyStringArray(selectedCreative?.descriptions)

  const effectiveKeywords =
    toKeywordTextArray(campaignConfig?.keywords).length > 0
  ...
  const effectiveCallouts =
    campaignConfigCallouts.length > 0 ? campaignConfigCallouts : selectedCreativeCallouts

  const effectiveSitelinks =
    Array.isArray(campaignConfig?.sitelinks) && campaignConfig.sitelinks.length > 0
      ? campaignConfig.sitelinks
      : Array.isArray(selectedCreative?.sitelinks)
        ? selectedCreative.sitelinks
        : []

  const parseApiError = (
    data: any
  ): { message: string; needsReauth: boolean; isRateLimited: boolean; retryAfter?: number } => {
    ...
    const message = isRateLimitedFlag
  ...
  console.log(
    `[Step4] selectedCreative.negativeKeywords长度: ${selectedCreative.negativeKeywords?.length || 0}`
  )
  console.log(
  ...
  const addPublishStep = (
    ...
    setPublishSteps((prev) => {
      ...
      if (existing) {
        return prev.map((s) =>
          s.step === step ? { ...s, message, status, timestamp: new Date() } : s
        )
      }
  ...
  const addServerWarnings = (warnings: unknown) => {
    ...
    if (warningMessages.length > 0) {
      addPublishStep(
        'warnings',
        `提示: ${warningMessages.join('; ').replace(/\[警告\]\s*/g, '')}`,
        'warning'
      )
    }
  }
  ...
  const pollCampaignCreationStatus = async (
    campaignIds: number[],
      ...
      for (const campaignId of campaignIds) {
        ...
        try {
          const statusRes = await fetch(
            `/api/offers/${offer.id}/campaigns/status?campaignId=${campaignId}`,
            {
              credentials: 'include',
              cache: 'no-store',
            }
  ...
  const addCampaignPublishFailureDetails = (
    ...
    const hasPolicyViolation = failedDetails.some(
      (d) => typeof d.error === 'string' && d.error.includes('政策违规')
    )
  ...
  const buildBatchSummary = (
    ...
    const forcePublishAccountIds = results
      .filter(
        (item) =>
          item.status === 'pending_confirmation' && item.confirmationAction === 'force_publish'
      )
    ...
    const pauseOldCampaignAccountIds = results
      .filter(
        (item) =>
          ...
          item.confirmationAction === 'pause_old_campaigns'
      )
      .map((item) => item.accountId as number)
      ...
      if (!response.ok) {
        if (apiError.needsReauth || response.status === 401) {
          return {
            ...
            accountName,
            status: 'failed',
            message: 'Google Ads 授权已过期，请重新授权后重试',
          }
        }
      ...
      if (response.status === 202) {
        ...
        if (campaignIds.length === 0) {
          return {
            ...
            accountName,
            status: 'warning',
            message: '任务已提交，但未返回Campaign列表',
          }
        }

        const { pendingIds, failedIds, failedDetails, successIds, warnings } =
          await pollCampaignCreationStatus(campaignIds)
  ...
  const executeBatchPublish = async (
    ...
    const workers = Array.from({ length: concurrency }, async () => {
      while (true) {
        ...
        addPublishStep(
          stepKey,
          `${accountLabel}：${result.message}`,
          mapResultStatusToStepStatus(result.status)
        )
      }
    })
  ...
  const finalizeBatchPublishStatus = (
    results: Array<{
      accountId: number
      customerId: string
      accountName: string
    ...
    addPublishStep(
      'preparing',
      `发布准备完成（${accountsToPublish.length} 个账号）`,
      preparingStepStatus
    )
    ...
    if (summary.forcePublishAccountIds.length > 0)
      pendingParts.push(`低分待强制 ${summary.forcePublishAccountIds.length}`)
    if (summary.pauseOldCampaignAccountIds.length > 0)
      pendingParts.push(`旧系列待确认 ${summary.pauseOldCampaignAccountIds.length}`)
  ...
  const handleForcePublish = async () => {
    try {
      ...
      if (response.status === 422 && data.action === 'LAUNCH_SCORE_BLOCKED') {
        ...
        addPublishStep(
          'creating',
          `投放评分过低 (${details.launchScore || 0}分)，无法强制发布`,
          'failed'
        )
        setPublishStatus({
          step: 'failed',
      ...
      if (response.status === 202) {
        ...
        if (campaignIds.length === 0) {
          addPublishStep(
            'syncing',
            '发布任务已提交，但未返回Campaign列表，请稍后刷新查看结果',
            'warning'
          )
          setPublishStatus({
            step: 'timeout',
            message:
              '发布任务已提交，但返回的Campaign列表为空；请稍后刷新页面或在Google Ads中确认结果。',
            success: false,
          })
          setPublishing(false)
          return
        ...
        if (pendingIds.length > 0) {
          addPublishStep(
            'syncing',
            `同步耗时较长：已完成 ${successIds.length}/${campaignIds.length}，仍在后台处理中`,
            'warning'
          )
          setPublishStatus({
            step: 'timeout',
            message:
              '处理耗时较长但后台仍在继续发布；请稍后点击“继续检查状态”或刷新页面查看最终结果。',
            success: false,
          })
          setPublishing(false)
          return
        ...
        addPublishStep(
          'completed',
          `${successIds.length}个广告系列已成功发布到Google Ads`,
          'success'
        )
        if (warnings.length > 0) {
          addPublishStep(
            'warnings',
            `提示: ${warnings.join('; ').replace('[警告] ', '')}`,
            'warning'
          )
        }
  ...
  const handlePublish = async () => {
    try {
      ...
      if (response.status === 422 && data.action === 'LAUNCH_SCORE_BLOCKED') {
        ...
        addPublishStep(
          'creating',
          `投放评分过低 (${details.launchScore || 0}分)，发布被阻止`,
          'failed'
        )
        setPublishStatus({
          step: 'failed',
      ...
      if (response.status === 422 && data.action === 'LAUNCH_SCORE_WARNING') {
        ...
        addPublishStep(
          'creating',
          `投放评分偏低 (${details.launchScore || 0}分)，建议优化`,
          'failed'
        )
        setPublishStatus({
          step: 'failed',
      ...
      if (response.status === 409) {
        ...
        const errorMessage =
          data.message || data.error?.error?.message || 'Ads账号已被其他Offer占用'
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>请仔细检查以下配置信息，确认无误后点击"发布广告"按钮</CardDescription>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="flex items-start space-x-3 p-3 bg-white rounded-lg border hover:border-blue-300 transition-colors">
                  ...
                  <div className="flex-1">
                    ...
                    >
                      立即启用新广告系列
                      <Badge
                        variant={enableCampaignImmediately ? 'default' : 'outline'}
                        className="text-xs"
                      >
                ...
                <div className="flex items-start space-x-3 p-3 bg-white rounded-lg border hover:border-blue-300 transition-colors">
                  ...
                  <div className="flex-1">
                    ...
                    >
                      暂停所有旧广告系列
                      <Badge
                        variant={pauseOldCampaigns ? 'destructive' : 'outline'}
                        className="text-xs"
                      >
              ...
              >
                {publishing ? (
                  <>
                    ...
                    {accountsToPublish.length > 1
                      ? `批量发布中 (${accountsToPublish.length}个账号)...`
                      : '发布中...'}
                  </>
                ) : (
                  <>
                    ...
                    {accountsToPublish.length > 1
                      ? `发布到 ${accountsToPublish.length} 个账号`
                      : '发布广告'}
                  </>
                )}
              </Button>
        ...
        {/* 右列：发布结果卡片 - 始终显示 */}
        <Card
          className={`border-2 lg:h-[400px] flex flex-col ${
                  ...
                  <AlertDescription className="space-y-3">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="text-sm text-amber-800">
                        ...
                        <div className="text-amber-700 mt-1">{rateLimitInfo.message}</div>
                        <div className="text-amber-700 mt-2">
                          <div className="flex items-center gap-2">
                            <span>预计需要等待:</span>
                            <span className="font-mono font-semibold bg-amber-100 px-2 py-0.5 rounded">
                              {Math.floor(rateLimitInfo.retryAfter / 3600)} 小时{' '}
                              {Math.floor((rateLimitInfo.retryAfter % 3600) / 60)} 分钟
                            </span>
                          </div>
                        </div>
                        <div className="text-amber-700 mt-3 pt-3 border-t border-amber-200">
                          ...
                          <ul className="text-xs space-y-1 text-amber-700 list-disc list-inside">
                            ...
                            <li>
                              建议稍后（{Math.ceil(rateLimitInfo.retryAfter / 60)}{' '}
                              分钟后）再尝试发布
                            </li>
                            <li>
                              如需更高配额，可访问{' '}
                              <a
                                href="https://support.google.com/google-ads/contact/quotas"
                                target="_blank"
                ...
                <Alert className="border-blue-200 bg-blue-50">
                  <AlertDescription className="space-y-3">
                    ...
                    <div className="text-xs text-blue-800 space-y-1">
                      {batchConfirmState.forcePublishAccountIds.length > 0 && (
                        <div>
                          低分待强制发布：{batchConfirmState.forcePublishAccountIds.length} 个账号
                        </div>
                      )}
                      {batchConfirmState.pauseOldCampaignAccountIds.length > 0 && (
                        <div>
                          待确认暂停旧系列：{batchConfirmState.pauseOldCampaignAccountIds.length}{' '}
                          个账号
                        </div>
                      )}
                    </div>
                ...
                <>
                                ...
                                onClick={async () => {
                                  try {
                                    ...
                                    const {
                                      ...
                                      successIds,
                                      warnings,
                                    } = await pollCampaignCreationStatus(lastPublishCampaignIds, {
                                      maxWaitMs: 120_000,
                                    })
                                    ...
                                    if (pendingIds.length > 0) {
                                      addPublishStep(
                                        'syncing',
                                        `仍在后台处理中：已完成 ${successIds.length}/${lastPublishCampaignIds.length}`,
                                        'warning'
                                      )
                                      setPublishStatus({
                                        step: 'timeout',
                                        message: '处理仍在继续；请稍后再检查或刷新页面。',
                                        success: false,
                                      })
                                      return
                                    }
                    ...
                    <div className="mt-4 p-4 bg-red-50 rounded-lg border border-red-200">
                      ...
                      <div className="flex items-center justify-between mb-4 pb-3 border-b border-red-200">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-semibold text-red-800">投放评分不足</span>
                        </div>
                      ...
                      {/* 各维度得分 */}
                      {launchScoreBlockDetails.breakdown &&
                        Object.keys(launchScoreBlockDetails.breakdown).length > 0 && (
                          <div className="mb-3">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              {launchScoreBlockDetails.breakdown.launchViability && (
                                <div className="flex items-center justify-between p-2 bg-white rounded border">
                                  ...
                                  <span className="text-xs font-semibold text-gray-800">
                                    {launchScoreBlockDetails.breakdown.launchViability.score}/
                                    {launchScoreBlockDetails.breakdown.launchViability.max}
                                  </span>
                                </div>
                              )}
                              {launchScoreBlockDetails.breakdown.adQuality && (
                                <div className="flex items-center justify-between p-2 bg-white rounded border">
                                  ...
                                  <span className="text-xs font-semibold text-gray-800">
                                    {launchScoreBlockDetails.breakdown.adQuality.score}/
                                    {launchScoreBlockDetails.breakdown.adQuality.max}
                                  </span>
                                </div>
                              )}
                              {launchScoreBlockDetails.breakdown.keywordStrategy && (
                                <div className="flex items-center justify-between p-2 bg-white rounded border">
                                  ...
                                  <span className="text-xs font-semibold text-gray-800">
                                    {launchScoreBlockDetails.breakdown.keywordStrategy.score}/
                                    {launchScoreBlockDetails.breakdown.keywordStrategy.max}
                                  </span>
                                </div>
                              )}
                              {launchScoreBlockDetails.breakdown.basicConfig && (
                                <div className="flex items-center justify-between p-2 bg-white rounded border">
                                  ...
                                  <span className="text-xs font-semibold text-gray-800">
                                    {launchScoreBlockDetails.breakdown.basicConfig.score}/
                                    {launchScoreBlockDetails.breakdown.basicConfig.max}
                                  </span>
                                </div>
                              )}
                            </div>
                          ...
                          <div className="mb-3">
                            ...
                            <ul className="space-y-1">
                              {launchScoreBlockDetails.issues.slice(0, 5).map((issue, idx) => (
                                <li
                                  key={idx}
                                  className="text-xs text-gray-600 flex items-start gap-2 p-1.5 bg-white rounded"
                                >
                      ...
                      {/* 改进建议 */}
                      {launchScoreBlockDetails.suggestions &&
                        launchScoreBlockDetails.suggestions.length > 0 && (
                          <div className="mb-3">
                            <div className="text-xs font-semibold text-gray-700 mb-2 flex items-center gap-1">
                    ...
                    <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                      ...
                      <div className="flex items-center justify-between mb-3 pb-3 border-b border-blue-200">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-semibold text-blue-800">投放评分</span>
                        </div>
                        <div className="text-right">
                          <div
                            className={`text-2xl font-bold ${getScoreColor(launchScoreSuccess.totalScore)}`}
                          >
                      ...
                      {/* 各维度得分 */}
                      {launchScoreSuccess.breakdown &&
                        Object.keys(launchScoreSuccess.breakdown).length > 0 && (
                          <div className="mb-3">
                            <div className="text-xs font-semibold text-gray-700 mb-2">
                              各维度得分
                            </div>
                            <div className="grid grid-cols-2 gap-2">
                              {launchScoreSuccess.breakdown.launchViability && (
                                <div className="flex items-center justify-between p-2 bg-white rounded border">
                                  <span className="text-xs text-gray-600">投放可行性</span>
                                  <span className="text-xs font-semibold text-gray-800">
                                    {launchScoreSuccess.breakdown.launchViability.score}/
                                    {launchScoreSuccess.breakdown.launchViability.max}
                                  </span>
                                </div>
                              )}
                              {launchScoreSuccess.breakdown.adQuality && (
                                <div className="flex items-center justify-between p-2 bg-white rounded border">
                                  <span className="text-xs text-gray-600">广告质量</span>
                                  <span className="text-xs font-semibold text-gray-800">
                                    {launchScoreSuccess.breakdown.adQuality.score}/
                                    {launchScoreSuccess.breakdown.adQuality.max}
                                  </span>
                                </div>
                              )}
                              {launchScoreSuccess.breakdown.keywordStrategy && (
                                <div className="flex items-center justify-between p-2 bg-white rounded border">
                                  <span className="text-xs text-gray-600">关键词策略</span>
                                  <span className="text-xs font-semibold text-gray-800">
                                    {launchScoreSuccess.breakdown.keywordStrategy.score}/
                                    {launchScoreSuccess.breakdown.keywordStrategy.max}
                                  </span>
                                </div>
                              )}
                              {launchScoreSuccess.breakdown.basicConfig && (
                                <div className="flex items-center justify-between p-2 bg-white rounded border">
                                  <span className="text-xs text-gray-600">基础配置</span>
                                  <span className="text-xs font-semibold text-gray-800">
                                    {launchScoreSuccess.breakdown.basicConfig.score}/
                                    {launchScoreSuccess.breakdown.basicConfig.max}
                                  </span>
                                </div>
                              )}
                            </div>
                      ...
                      {/* 优化建议（如果有） */}
                      {launchScoreSuccess.overallRecommendations &&
                        launchScoreSuccess.overallRecommendations.length > 0 && (
                  ...
                  {publishStatus?.success && (
                    <div className="mt-4 p-3 bg-green-50 rounded-lg border border-green-200">
                      <div className="flex items-center gap-2">
                        <CheckCircle2 className="w-4 h-4 text-green-600" />
                        <span className="text-sm text-green-800">广告系列已成功发布</span>
                      </div>
                    </div>
                  )}
                </>
              ...
              {selectedCreative.adStrength?.dimensions ? (
                <>
                  <div>
                    ...
                    <span className="font-semibold">
                      {selectedCreative.adStrength.dimensions.relevance.score}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600">质量:</span>{' '}
                    <span className="font-semibold">
                      {selectedCreative.adStrength.dimensions.quality.score}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600">吸引力:</span>{' '}
                    <span className="font-semibold">
                      {selectedCreative.adStrength.dimensions.completeness.score}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600">多样性:</span>{' '}
                    <span className="font-semibold">
                      {selectedCreative.adStrength.dimensions.diversity.score}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600">合规性:</span>{' '}
                    <span className="font-semibold">
                      {selectedCreative.adStrength.dimensions.compliance.score}
                    </span>
                  </div>
                  {selectedCreative.adStrength.dimensions.brandSearchVolume && (
                    <div>
                      ...
                      <span className="font-semibold">
                        {selectedCreative.adStrength.dimensions.brandSearchVolume.score}
                      </span>
                    </div>
                  )}
                  {selectedCreative.adStrength.dimensions.competitivePositioning && (
                    <div>
                      ...
                      <span className="font-semibold">
                        {selectedCreative.adStrength.dimensions.competitivePositioning.score}
                      </span>
                    </div>
                  )}
                </>
                ...
                <>
                  <div>
                    <span className="text-gray-600">相关性:</span>{' '}
                    <span className="font-semibold">
                      {selectedCreative.scoreBreakdown.relevance}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600">质量:</span>{' '}
                  ...
                  <div>
                    <span className="text-gray-600">吸引力:</span>{' '}
                    <span className="font-semibold">
                      {selectedCreative.scoreBreakdown.engagement}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600">多样性:</span>{' '}
                    <span className="font-semibold">
                      {selectedCreative.scoreBreakdown.diversity}
                    </span>
                  </div>
                  <div>
            ...
            <div>
              ...
              <div className="space-y-1 text-sm text-gray-600">
                ...
                {effectiveHeadlines.length > 5 && (
                  <div className="text-gray-400">+{effectiveHeadlines.length - 5} 更多...</div>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-1 text-sm text-gray-600">
                ...
                {effectiveSitelinks.length > 4 && (
                  <div className="text-gray-400">+{effectiveSitelinks.length - 4} 更多...</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4 text-sm">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="font-semibold mt-1">
                  {currencySymbol}
                  {campaignConfig.maxCpcBid.toFixed(2)}
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="font-semibold mt-1">
                  {campaignConfig.negativeKeywords.length} 个
                </div>
              </div>
            </div>
          </div>
            ...
            {accountsToPublish.map((account: any) => (
              ...
              >
                <div>
                  ...
                  <div className="text-sm text-gray-600 font-mono mt-1">{account.customerId}</div>
                </div>
      ...
      <Dialog open={showPauseConfirm} onOpenChange={setShowPauseConfirm}>
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>{pauseConfirmMessage}</DialogDescription>
          </DialogHeader>
                ...
                <TableBody>
                  {existingCampaigns.map((camp: any) => (
                    <TableRow key={camp.id}>
                      ...
                      <TableCell>
                        {typeof camp.budget === 'number' ? `${currencySymbol}${camp.budget}` : '-'}
                      </TableCell>
          ...
          <DialogFooter className="flex flex-col gap-2 sm:flex-row sm:justify-end">
            ...
            <Button variant="default" onClick={handlePublishTogether} disabled={publishing}>
      ...
      <Dialog open={showForcePublishConfirm} onOpenChange={setShowForcePublishConfirm}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              该Offer的投放评分为 {launchScoreBlockDetails?.launchScore}分，低于建议值
              {launchScoreBlockDetails?.threshold}分
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="flex gap-2">
            ...
            <Button variant="destructive" onClick={handleForcePublish} disabled={publishing}>
