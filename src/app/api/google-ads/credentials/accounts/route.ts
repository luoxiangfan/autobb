...
const get = withAuth(async (request, user) => {
  try {
    ...
    accountsLog.debug('accounts_request', {
      userId,
      forceRefresh,
      asyncRefresh,
      ...
      accounts.map((acc) => {
        ...
        return {
          ...
          identity_verification_completion_deadline_time:
            acc.identity_verification_completion_deadline_time,
    ...
    let effectiveLastSyncAtIso: string | null = Number.isNaN(latestSyncAtMs)
      ? null
      : new Date(latestSyncAtMs).toISOString()
    ...
    if (forceRefresh && asyncRefresh) {
      ...
      accountsLog.debug('sync_started', {
        userId,
        forceRefresh,
      ...
      if (startResult.started) {
        try {
          ...
          accountsLog.debug('sync_completed', { userId, accountCount: allAccounts.length })
        ...
        usedCache = true
        accountsLog.debug('sync_in_progress_using_cache', {
          userId,
          cachedCount: cachedAccounts.length,
        })
        allAccounts = mapCachedAccounts()
        ...
        if (waitedAccounts.length > 0) {
          ...
          if (isGoogleAdsAccountRefreshInProgress(stateAfterWait)) {
            usedCache = true
            allAccounts = cachedAccounts.length > 0 ? mapCachedAccounts() : []
            accountsLog.debug('sync_in_progress_poll', { userId })
          } else {
      ...
      `,
        ...
        if (offerId && linkedOffersMapped.length > 0) {
          ...
          if (usedByCurrentOffer) {
            ...
            // 检查是否被同品牌Offer使用过
            const usedBySameBrand = linkedOffersMapped.some(
              (o: any) => o.brand === currentOfferBrand
            )
            if (usedBySameBrand) {
              priority = 'same-brand'
              priorityScore = 1
    ...
    if (filterByUserMcc && !isAdmin) {
      ...
      accountsLog.debug('filter_by_user_mcc', {
        userId,
        role: 'user',
    ...
    return jsonNoStore({
      ...
      data: {
        ...
        refreshError:
          finalSyncState?.status === 'failed' ? finalSyncState.errorMessage || null : null,
        refreshStartedAt: finalSyncState?.startedAtMs
          ? new Date(finalSyncState.startedAtMs).toISOString()
          : null,
    ...
    accountsLog.error('get_accounts_failed', {}, error)
    ...
    if (
      ...
      (httpStatus === 401 && error.message === 'Request failed')
    ) {
      statusCode = 401
      ...
      return jsonNoStore(
        {
          ...
          message:
            ...
            'Google OAuth client_id/client_secret 配置错误或已失效，请在设置页面重新配置后再授权',
          needsReauth: false,
          solution: {
            title: '如何修复',
            steps: [
    ...
    if (
      ...
      (extractedMessageLower.includes('developer token') &&
        extractedMessageLower.includes('not approved'))
    ) {
      statusCode = 403
    ...
    if (
      (extractedMessageLower.includes('developer token') &&
        extractedMessageLower.includes('not valid')) ||
