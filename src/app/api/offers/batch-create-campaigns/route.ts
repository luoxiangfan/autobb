...
export async function POST(request: NextRequest) {
  try {
    ...
    const { offerIds, googleAdsAccountId, campaignConfig } = body
    ...
    if (!adsAccount) {
      return NextResponse.json({ error: 'Google Ads 账号不存在或无权访问' }, { status: 404 })
    }
    ...
    if (!isActive || isDeleted) {
      return NextResponse.json({ error: 'Google Ads 账号已禁用或删除' }, { status: 400 })
    }
    ...
    for (const offerId of offerIds) {
      try {
        ...
        `,
          [offerId, userId]
        )) as { id: number; creative_name: string; keyword_bucket: string } | undefined
        ...
        const dateStr = new Date()
          ...
          .split('T')[0]
          .replace(/[-t:z.]/gi, '')
        const brandName = (offer.brand || offer.offer_name || 'Unknown')
          .replace(/[^a-zA-Z0-9]/g, '')
          .slice(0, 20)
