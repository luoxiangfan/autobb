...
export const POST = withAuth(async (request: NextRequest, user) => {
    ...
    return NextResponse.json(
      {
        ...
        message: 'JSON格式错误: ' + (parseError instanceof Error ? parseError.message : '未知错误'),
      },
      { status: 400 }
    )
  }
  ...
  console.log('[CreateClickFarmTask] 解析后的请求数据:', {
    ...
    hourly_distribution_length: Array.isArray(body.hourly_distribution)
      ? body.hourly_distribution.length
      : 'not-array',
  })
  ...
  if (normalizedStartTime && !timeFormatRegex.test(normalizedStartTime)) {
    return NextResponse.json(
      {
        error: 'validation_error',
        message: '开始时间格式无效，请使用 HH:mm 或 HH:mm:ss 或 24:00',
      },
      { status: 400 }
    )
  }
  if (normalizedEndTime && !timeFormatRegex.test(normalizedEndTime)) {
    return NextResponse.json(
      {
        error: 'validation_error',
        message: '结束时间格式无效，请使用 HH:mm 或 HH:mm:ss 或 24:00',
      },
      { status: 400 }
    )
  }
  ...
  if (scheduledDate === todayInTaskTimezone) {
    ...
    const backgroundAlive = routingDiagnostics.splitEnabled ? await isBackgroundWorkerAlive() : true
    ...
    console.log(
      `[CreateTask] scheduledDate=${scheduledDate}, todayInTaskTimezone=${todayInTaskTimezone}`
    )
  }
      ...
      message:
        triggerResult?.status === 'queued'
          ...
          : triggerResult?.mode === 'queue_unavailable' ||
              triggerResult?.reason === 'queue_unavailable'
            ? '补点击任务创建成功，但队列未就绪，调度将由后台稍后处理'
            : triggerResult?.mode === 'worker_unavailable' ||
                triggerResult?.reason === 'worker_unavailable'
...
export const GET = withAuth(async (request: NextRequest, user) => {
  ...
  const filters: TaskFilters = {
    ...
    include_deleted: parseBooleanQuery(
  ...
  const result = await getClickFarmTasks(user.userId, filters)
