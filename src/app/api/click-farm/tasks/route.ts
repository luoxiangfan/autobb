...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          message:
            'JSON格式错误: ' + (parseError instanceof Error ? parseError.message : '未知错误'),
        },
        { status: 400 }
      )
    }
    ...
    const normalizedEndTime =
      typeof body.end_time === 'string' ? body.end_time.trim() : body.end_time
    ...
    if (scheduledDate === todayInTaskTimezone) {
      ...
      if (!redisReady) {
        ...
        console.log(
          `[CreateTask] 任务 ${task.id} 开始日期是今天，跳过立即调度 (mode=${triggerMode})`
        )
      } else {
        try {
