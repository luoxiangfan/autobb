...
const runsQuerySchema = z.object({
  page: z.coerce.number().int(zErr.int).min(1, zErr.minNumber(1)).optional(),
  limit: z.coerce.number().int(zErr.int).min(1, zErr.minNumber(1)).max(100, zErr.maxNumber(100)).optional(),
  ...
  try {
    ...
    if (!parsedQuery.success) {
      return NextResponse.json(
        { error: parsedQuery.error.issues[0]?.message || '请求参数错误' },
        { status: 400 }
      )
    }
