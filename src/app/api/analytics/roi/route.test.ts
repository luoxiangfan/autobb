...
describe('GET /api/analytics/roi', () => {
  beforeEach(() => {
    ...
    dbFns.query.mockImplementation(async (sql: string) => {
      ...
      if (
        sql.includes("SELECT DISTINCT COALESCE(currency, 'USD') as currency") &&
        sql.includes('affiliate_commission_attributions')
      ) {
        return [{ currency: 'USD' }]
      }
      if (
      ...
      if (
        sql.includes('report_date as date') &&
        sql.includes('affiliate_commission_attributions')
      ) {
      ...
      if (
        sql.includes('campaign_id') &&
        sql.includes('affiliate_commission_attributions') &&
        sql.includes('GROUP BY campaign_id')
      ) {
      ...
      if (
        sql.includes('o.id as offer_id') &&
        sql.includes('COUNT(DISTINCT c.id) as campaign_count')
      ) {
        return [
          { offer_id: 11, brand: 'Brand A', offer_name: 'Offer A', campaign_count: 1, cost: 100 },
        ]
      }
      if (
        sql.includes('offer_id') &&
        sql.includes('affiliate_commission_attributions') &&
        sql.includes('GROUP BY offer_id')
      ) {
  ...
  it('uses affiliate commission totals to compute revenue and roas', async () => {
    const req = new NextRequest(
      'http://localhost/api/analytics/roi?start_date=2026-04-01&end_date=2026-04-01'
    )
    ...
    expect(data.data.byCampaign[0]).toEqual(
    ...
    expect(data.data.byOffer[0]).toEqual(
      expect.objectContaining({
        offerId: 11,
        revenue: 50,
        conversions: 3,
