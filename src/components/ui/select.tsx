...
export const Select = React.forwardRef<HTMLSelectElement, SelectProps & { onValueChange?: (value: string) => void }>(
  ({ className = '', padding = 'py-2', children, onValueChange, ...props }, ref) => {
    return (
      <select
        ref={ref}
        className={`flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 ${padding} text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-hidden focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 ${className}`}
