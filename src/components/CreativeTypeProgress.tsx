...
export function CreativeTypeProgress({
  generatedBuckets,
  activeBucket,
  offer,
}: CreativeTypeProgressProps) {
        ...
        <div className="flex items-center justify-between gap-3">
                ...
                <Tooltip key={bucket.key}>
                    ...
                    >
                      <div
                        className={`
                          ...
                          ${
                            isGenerated ? `${bucket.color} text-white` : 'bg-gray-100 text-gray-400'
                          }
                      ...
                      <span
                        className={`text-xs font-semibold ${isGenerated ? 'text-gray-900' : 'text-gray-500'}`}
                      >
                        {bucket.label}
                      </span>
                  ...
                  <TooltipContent>
                    ...
                    <p className="text-[11px] text-gray-500">
                      槽位 {bucket.key} · {bucket.creativeType}
                    </p>
                    <p className="text-xs text-gray-500">{bucket.description}</p>
                    {isGenerated && <p className="mt-1 text-xs text-green-600">✓ 已生成</p>}
                    {isGenerating && <p className="mt-1 text-xs text-purple-600">生成中</p>}
          ...
          <span className="text-xs text-gray-500">
            {generatedBuckets.length === 3 ? (
              ...
              <span className="font-medium text-purple-600">生成中</span>
            ) : generatedBuckets.length === 0 ? (
              <span className="text-gray-400">未开始</span>
            ) : (
              <span className="text-gray-600">{generatedBuckets.length}/3</span>
            )}
          </span>
        </div>
      </div>
