...
class SchemaValidator {
  ...
  private testPromptVersions(db: Database.Database): ValidationResult {
    const totalVersions = db.prepare('SELECT COUNT(*) as count FROM prompt_versions').get() as {
      count: number
    }
    const activeVersions = db
      .prepare('SELECT COUNT(*) as count FROM prompt_versions WHERE is_active = 1')
      .get() as { count: number }
    const promptTypes = db
      .prepare('SELECT COUNT(DISTINCT prompt_id) as count FROM prompt_versions')
      .get() as { count: number }
  ...
  private testCriticalColumns(db: Database.Database): ValidationResult {
    ...
    for (const check of checks) {
      try {
        const columns = db.prepare(`PRAGMA table_info(${check.table})`).all() as Array<{
          name: string
        }>
