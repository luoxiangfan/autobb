...
async function runPendingMigrations(sql: ReturnType<typeof postgres>): Promise<void> {
  ...
  console.log(
    `⏱️  增量迁移执行配置: lock_timeout=${STARTUP_MIGRATION_LOCK_TIMEOUT_MS}ms, statement_timeout=${STARTUP_MIGRATION_STATEMENT_TIMEOUT_MS}ms, budget=${STARTUP_MIGRATION_MAX_DURATION_MS}ms`
  )
  ...
  for (const item of pendingToRun) {
    ...
    try {
      await sql.begin(async (tx) => {
        ...
        await tx.unsafe(
          `SET LOCAL statement_timeout = '${STARTUP_MIGRATION_STATEMENT_TIMEOUT_MS}ms'`
        )
        ...
        for (const stmt of statements) {
          ...
          try {
            ...
            if (errorMsg.includes('canceling statement due to statement timeout')) {
              throw new Error(
                `语句触发 statement_timeout(${STARTUP_MIGRATION_STATEMENT_TIMEOUT_MS}ms): ${trimmed.substring(0, 120)}...`
              )
            }
