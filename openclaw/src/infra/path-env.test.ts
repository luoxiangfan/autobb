...
describe("ensureOpenClawCliOnPath", () => {
  ...
  it("appends project-local node_modules/.bin only when explicitly enabled", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-path-"));
    const originalPath = process.env.PATH;
    const originalFlag = process.env.OPENCLAW_PATH_BOOTSTRAPPED;
    const originalAllowLocal = process.env.OPENCLAW_ALLOW_PROJECT_LOCAL_BIN;
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const appCli = path.join(appBinDir, "openclaw");
      ...
      if (originalAllowLocal === undefined) {
        ...
        process.env.OPENCLAW_ALLOW_PROJECT_LOCAL_BIN = originalAllowLocal;
      }
      await fs.rm(tmp, { recursive: true, force: true });
    }
  });
